...
SIMS.opensharing = (root)=>{
  ...
  function draw(){
    ...
    ctx.fillStyle='#04121a';ctx.font='600 12px Inter';ctx.textAlign='center';ctx.fillText('Provider',px,py-6);
    ctx.fillText('Unity Catalog',px,py+10);
    ...
    if(sc){ ctx.fillStyle=COL.amber; roundRect(ctx,proxyX-34,py-22,68,44,9);ctx.fill();
      ctx.fillStyle='#1a1200';ctx.fillText('Secure',proxyX,py-2);ctx.fillText('Connect',proxyX,py+12);}
    // recipients
    ...
    for(let i=0;i<recips;i++){
      ...
      ctx.fillStyle='#fff';ctx.font='600 10px Inter';ctx.fillText('R'+(i+1),rx,ry-1);
      ctx.fillText(replica?'replica':'remote',rx,ry+10);
    }
    // legend / status
    ctx.textAlign='left';ctx.font='11px Inter';ctx.fillStyle=COL.ink3;
    ctx.fillText((sc?'1 config for all':(recips+' manual allowlists'))+(gd?' · 0 egress':''),10,H-10);
...
SIMS.unitycatalog = (root)=>{
  ...
  function draw(){const W=cv.clientWidth,H=240;ctx.clearRect(0,0,W,H);
    ...
    nodes.forEach(n=>{const p=pos(n);const on= sel===n.id;
      ...
      ctx.fillStyle=cssv('--ink');ctx.font='600 11px Inter';ctx.textAlign='center';ctx.fillText(n.l,p.x,p.y-16);});
  }
...
SIMS.lakeflow = (root)=>{
  ...
  function draw(){const W=cv.clientWidth,H=160;ctx.clearRect(0,0,W,H);const y=H/2;
    ...
    stages.forEach((s,i)=>{const x=gap*(i+0.5);ctx.fillStyle=s.c;roundRect(ctx,x-38,y-22,76,44,9);ctx.fill();
      ctx.fillStyle='#06121a';ctx.font='600 12px Inter';ctx.textAlign='center';ctx.fillText(s.l,x,y+4);});
    raf=requestAnimationFrame(draw);}
  ...
  function draw(){const W=cv.clientWidth,H=220;ctx.clearRect(0,0,W,H);ctx.textAlign='center';ctx.font='600 12px Inter';
    if(mode==='legacy'){
      ...
      boxes.forEach((b,i)=>{const x=b.x*W;ctx.fillStyle=b.c;roundRect(ctx,x-44,H/2-26,88,52,10);ctx.fill();
        ctx.fillStyle='#06121a';ctx.fillText(b.l,x,H/2+4);
      ...
      ctx.fillStyle=COL.bad;ctx.fillText('copy → copy → copy',W/2,H-14);
    } else {
      // one storage with two engines
      const cx=W/2;ctx.fillStyle=COL.pink;roundRect(ctx,cx-90,H-58,180,40,10);ctx.fill();
      ctx.fillStyle='#1a0a10';ctx.fillText('One copy · Delta/Iceberg · Unity Catalog',cx,H-32);
      const oltp={x:cx-110,y:60},olap={x:cx+110,y:60};
      [['Lakebase (OLTP)',oltp,COL.blue],['Lakehouse (OLAP)',olap,COL.amber]].forEach(([l,p,c])=>{
        ctx.fillStyle=c;roundRect(ctx,p.x-70,p.y-22,140,44,10);ctx.fill();ctx.fillStyle='#06121a';ctx.fillText(l,p.x,p.y+4);
      ...
      if(branch){ctx.fillStyle=COL.green;roundRect(ctx,cx+150,H-58,70,40,10);ctx.fill();
        ctx.fillStyle='#06140c';ctx.fillText('branch',cx+185,H-32);
  ...
  function draw(){
    ...
    if(mode==='lake'){
      ...
      ['Raw files','Streams','Tables'].forEach((s,i)=>{const sy=70+i*70;
        ...
        ctx.fillStyle=cssv('--ink2');ctx.font='600 11px Inter';ctx.fillText(s,srcX,sy+4);
      ...
      ctx.fillStyle=COL.amber;ctx.font='10px Inter';ctx.fillText('Unity Catalog governance',cx,cy-82);
      ctx.fillStyle=COL.pink;roundRect(ctx,cx-58,cy-34,116,68,14);ctx.fill();
      ctx.fillStyle='#1a0a10';ctx.font='700 14px Space Grotesk';ctx.fillText('Lakehouse',cx,cy-3);
      ctx.font='10px Inter';ctx.fillText('Delta Lake · Spark',cx,cy+14);
      const wx=W-78;
      WORK.forEach((w,i)=>{const wy=46+i*((H-92)/3),on=wl[w.k];
        ...
        ctx.fillStyle=on?'#16243a':'#1b2436';roundRect(ctx,wx-52,wy-16,104,32,8);ctx.fill();
        ctx.strokeStyle=on?COL.green:'#2a3852';ctx.lineWidth=1.5;roundRect(ctx,wx-52,wy-16,104,32,8);ctx.stroke();
        ctx.fillStyle=on?cssv('--ink'):cssv('--ink3');ctx.font='600 11px Inter';ctx.fillText(w.l,wx,wy+4);});
      ctx.fillStyle=cssv('--ink3');ctx.font='10px Inter';ctx.textAlign='right';ctx.fillText('runs on '+cloud,W-8,H-8);ctx.textAlign='center';
    } else {
      const lakeY=H*0.3, whY=H*0.72, leftX=110;
      ctx.fillStyle=COL.blue;roundRect(ctx,leftX-66,lakeY-26,132,52,12);ctx.fill();
      ctx.fillStyle='#06121a';ctx.font='700 13px Space Grotesk';ctx.fillText('Data Lake',leftX,lakeY-2);ctx.font='9px Inter';ctx.fillText('scalable · ungoverned',leftX,lakeY+13);
      ctx.fillStyle=COL.amber;roundRect(ctx,leftX-66,whY-26,132,52,12);ctx.fill();
      ctx.fillStyle='#1a1200';ctx.font='700 13px Space Grotesk';ctx.fillText('Warehouse',leftX,whY-2);ctx.font='9px Inter';ctx.fillText('governed · rigid',leftX,whY+13);
      ...
      ctx.fillStyle=COL.bad;ctx.font='10px Inter';ctx.fillText('ETL copy',leftX+44,(lakeY+whY)/2+3);
      const wx=W-86;
      WORK.forEach((w,i)=>{const wy=46+i*((H-92)/3),on=wl[w.k],fromWh=(w.k==='bi'||w.k==='eng'),sx=leftX+66,sy=fromWh?whY:lakeY;
        ...
        ctx.fillStyle='#1b2436';roundRect(ctx,wx-52,wy-15,104,30,8);ctx.fill();
        ctx.fillStyle=on?cssv('--ink2'):cssv('--ink3');ctx.font='600 11px Inter';ctx.fillText(w.l,wx,wy+4);});
      ctx.fillStyle=COL.bad;ctx.font='10px Inter';ctx.fillText('two copies · stranded workloads · brittle pipelines',W*0.52,H-8);
    }
    raf=requestAnimationFrame(draw);
  }
...
SIMS.geniontology = (root)=>{
  ...
  function draw(){const W=cv.clientWidth,H=240;ctx.clearRect(0,0,W,H);const now=performance.now();
    ...
    concepts.forEach((c,i)=>{const y=26+i*((H-52)/(concepts.length-1));const act=on[c.k];
      ...
      ctx.fillText(c.l,lx+12,y+3);});
    ctx.fillStyle=COL.gold;roundRect(ctx,cx-42,cy-22,84,44,10);ctx.fill();
    ctx.fillStyle='#1a1300';ctx.font='600 11px Inter';ctx.textAlign='center';
    ctx.fillText('Genie',cx,cy-2);ctx.fillText('Ontology',cx,cy+12);
    ...
    [['A',cy-58],['B',cy+58]].forEach(([l,y],idx)=>{
      ...
      ctx.fillStyle='#fff';ctx.font='600 13px Inter';ctx.textAlign='center';ctx.fillText(l,ax,y+5);});
    raf=requestAnimationFrame(draw);}
  function resize(){cv.width=cv.clientWidth*devicePixelRatio;cv.height=240*devicePixelRatio;
    ctx.setTransform(devicePixelRatio,0,0,devicePixelRatio,0,0);}
  window.addEventListener('resize',resize);onTeardown(()=>{cancelAnimationFrame(raf);window.removeEventListener('resize',resize);});
...
SIMS.secureconnect = (root)=>{
  ...
  function draw(){const W=cv.clientWidth,H=180;ctx.clearRect(0,0,W,H);const px=58,py=H/2;
    ...
    ctx.fillStyle='#04121a';ctx.font='600 11px Inter';ctx.textAlign='center';ctx.fillText('Provider',px,py+4);
    const proxyX=W*0.46;
    if(proxy){ctx.fillStyle=COL.cyan;roundRect(ctx,proxyX-28,py-17,56,34,9);ctx.fill();
      ctx.fillStyle='#04121a';ctx.fillText('proxy',proxyX,py+4);
    ...
    if(n>show){ctx.fillStyle=cssv('--ink3');ctx.font='10px Inter';ctx.textAlign='right';ctx.fillText('+'+(n-show)+' more',rx,H-3);}
  }
