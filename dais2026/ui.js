...
const MODULE_ICONS={
  ...
  lakehouse:'house', ltap:'db', lakehousert:'house', lakebasepartners:'db',
  lakeflow:'flow', aifirst:'flow',
  unitycatalog:'cog', aigateway:'agent', security:'cog', lakewatch:'watch',
  opensharing:'share', secureconnect:'share',
  aiplatform:'model', agentbricks:'bricks', omnigent:'flower',
  geniecode:'genie', genieone:'genie', genieagents:'genie', geniezeroops:'genie',
  vibecoding:'apps', geniontology:'lamp',
  aibi:'apps', marketplace:'apps', agenticcdp:'people', freeedition:'layers'
};
function productTile(ic, extraClass=''){
  const extra=extraClass ? ` ${extraClass}` : '';
  return OFFICIAL_PRODUCT_TILES.has(ic)
    ? `<span class="itile product-logo logo-${ic}${extra}" aria-hidden="true"></span>`
    : `<span class="itile${extra}" aria-hidden="true">${ICONS[ic]||ICONS.layers}</span>`;
}
function renderLanding(){
  const item=(route,ic,name,sm)=>`<div class="pditem${sm?' sm':''}" role="button" tabindex="0" data-route="${route}">
    ${productTile(ic)}<span class="iname">${name}</span></div>`;
...
function renderScreen(key){
  const c = tr(key);
  const moduleIcon = MODULE_ICONS[key] || 'layers';
  ...
  <section class="screen">
    <div class="shead" style="--module-color:var(${c.color})">
      <div class="module-mark">${productTile(moduleIcon,'detail-tile')}</div>
      <div class="shead-copy">
